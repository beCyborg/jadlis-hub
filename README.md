...
Одно слово в чате показывает семь строк маршрута 0 → 6 и ту, на которой ты стоишь сейчас;
...
целиком и ждёт твоего «да», а снимается всё одной командой — `claude plugin uninstall jadlis-start@jadlis --keep-data`.
...
|---|---|---|
| **Порядок шагов.** Инструментов много, порядка между ними нет: список открываешь и закрываешь, потому что выбирать из двадцати — это тоже работа. | Чат выдаёт все шаги сразу, и выбирать снова тебе. | Пишешь в чат `JADLIS-BATCH` — семь строк маршрута 0 → 6 со статусами и строка «где ты сейчас». |
...
На входе — слово `JADLIS-BATCH` в чате.
...
Текстом: слово в чате → маршрут 0 → 6 со статусами → проба на машине → шаг открывается; не
...
```
Ты — установщик. Поставь на этот Mac плагин jadlis-start из маркетплейса jadlis.
...
3. claude plugin list — покажи мне строку про jadlis-start и его версию.
Потом напиши в чат слово JADLIS-BATCH и покажи мне маршрут со статусами.
...
**в) Первое слово.** Открой Claude Code в папке, где работаешь, и напиши одно слово, без слэша:

```
JADLIS-BATCH
```

Это слово, а не команда со слэшем. Оно и открывает маршрут, и возвращает тебя в него после
перерыва, и повторяется в конце каждого ответа — искать его в переписке не придётся.

Первый шаг маршрута — рабочее место: маршрут сам поставит `setup@jadlis` и проведёт по его
шагам — `/setup:deps`, `/setup:workplace`, `/setup:obsidian` и, по желанию, `/setup:terminal`.
Ставить его отдельно заранее не нужно.
...
**Каталог.** Маршрут ведёт по остальным 17 репозиториям витрины: каждый ставится своей командой,
единой кнопки «поставить всё» нет и не будет — ставится то, до чего ты дошёл. Карта тиров с
доками к каждому — в [docs](docs/).
