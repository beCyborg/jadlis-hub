...
- README **хаба** (корень) — только русский, без EN-пары и переключателя: он для человека, которому шлют ссылку; полная ссылка на репо и блок для ИИ читателя (текст для вставки в чат) стоят в самом начале, до hero, картинки `docs/img/hub-*.webp` по палитре хаба из `docs/brand/identity.md`, тезис внутри картинки + одна строка «Зачем» текстом. Проверка: `python3 tools/readme-parity.py --allow-missing-en` (относительные ссылки).
