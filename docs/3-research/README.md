...
# Тир 3 · Ресерч: `search`, `full-research`, `search-paper`

Три инструмента одного плагина `jadlis-research`: быстрый веб-поиск, проверка утверждений по многим каналам и научный обзор с оценкой силы доказательства. Все числа ниже сверены с кодом плагина на теге `jadlis-research--v1.2.0`.
...
|---|---|---|
| «Что вообще известно о X?» | `/jadlis-research:search` | ответ со ссылками в чате + строка стоимости в лог |
| «Так ли это сейчас и что говорят практики?» | `/jadlis-research:full-research` | заметка в vault + леджер: что подтвердилось, что отсеяно и почему |
| «Насколько сильно это доказано?» | `/jadlis-research:search-paper` | отчёт с оценкой GRADE по каждому исходу |

Развилка между вторым и третьим — единица работы. У `full-research` это **утверждение**: его проверяют встречным поиском по источникам разного типа. У `search-paper` — **статья и её исход**: проверка библиографическая и методологическая.

Проблема, которую это решает: погуглил полчаса — и одно и то же в семи местах. Ощущается как подтверждение, а обычно это одна цитата, которую переписали все. Три поисковых движка — один тип источника, а не три. `full-research` отдельной фазой ищет, чем каждое утверждение можно опровергнуть, и в выводы пускает только выжившее. `search-paper` отвечает на другой вопрос: не «правда ли это», а «насколько сильно это доказано» — и оценка умеет понижаться с названной причиной.
...
![Две лупы над одной карточкой: через одну она целая, через другую треснула](../img/05-full-research-01.webp)
...
![Полка с пятью контейнерами; три одинаковых листа связаны скобой в один контейнер](../img/05-full-research-02.webp)
...
<details>
<summary>Фрагмент отчёта <code>full-research</code> (синтетический пример)</summary>
...
![Конвейер с листами проходит три арки: крюк снимает лист в закрытый ящик, штамп прижимается ко второму, третий проходит дальше](../img/06-search-paper-03.webp)

`search-paper` устроен иначе: 9 научных источников параллельно → снежный ком по цитированиям (до 6 хабов за круг, максимум 2 круга, потолок корпуса 120 работ) → гигиена ссылок (батчи по 25 DOI: существование, сверка заголовка, три сигнала отзыва) → GRADE по каждому исходу → независимый критик → правки с уверенностью от 0,7.

<details>
<summary>Фрагмент отчёта <code>search-paper</code> (синтетический пример)</summary>
...
Заведи научные ключи и внешние бинари — сам плагин уже стоит с тира 2.
...
2. Bash: claude plugin install jadlis-research@jadlis
   (если плагин уже стоит — пропусти шаг)
...
5. Запусти скилл /jadlis-research:keys и пройди его до таблицы PASS/FAIL.
...
Дальше `/jadlis-research:keys`: скилл принимает значения по одному, кладёт их в Связку ключей macOS (класс B: служба `jadlis-research`, account = имя ключа) и печатает смоук-таблицу PASS/FAIL по каждому источнику. Значения ключей не показываются нигде — ни в ответе, ни в эхо Bash.
...
Куда всё ложится: отчёты — в `VAULT_PATH/Знания/Ресерчи/` (`VAULT_PATH` задаётся при включении плагина, по умолчанию `~/Jadlis`), рабочие файлы прогона — в `VAULT_PATH/.full-research/<id>_<slug>/` и `VAULT_PATH/.search-paper/<id>_<slug>/`.
...
**1. Быстрый вопрос — `/jadlis-research:search`**

```text
/jadlis-research:search что известно о статических генераторах вики в 2026
```
...
**2. Проверка утверждений — `/jadlis-research:full-research`**

```text
/jadlis-research:full-research переносить ли командную вики на статический генератор
```
...
**3. Научный обзор — `/jadlis-research:search-paper`**

```text
/jadlis-research:search-paper мешает ли фоновая речь пониманию прочитанного
```
...
Повторный смоук ключей без записи: `/jadlis-research:keys --check`.
...
Сколько это стоит по модели. В `full-research` число вызовов агентов складывается детерминированно: каналы + 1 куратор + 1 urlhealth + 2 верификатора на каждое утверждение + до 8 эскалаций в Codex + 1 аналитик. Дефолтный прогон с 7 каналами и полным потолком в 16 утверждений — до 50 вызовов. Отдельной оценки в токенах или деньгах в коде нет, и выдумывать её тут не будем.
...
- `full-research` не проверяет весь текст: потолок — 16 утверждений за прогон, остальное в отчёте собрано, но не проверено.
...
- `full-research` не судит методологию исследований; вопрос «насколько сильно доказано» — к `search-paper`.
- `search-paper` ставит GRADE в основном по абстрактам: полные тексты читаются у 6 работ, а не у всего корпуса.
